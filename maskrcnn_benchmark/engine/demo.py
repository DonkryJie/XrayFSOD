...
def draw(pic_id, boxes, labels):
    pic = cv2.imread('/home/hl/students/DYJ/one/datasets/voc/VOC2007/JPEGImages/' + pic_id, cv2.IMREAD_COLOR)
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        ...
        with torch.no_grad():
            ...
            if cfg.TEST.BBOX_AUG.ENABLED:
                ...
                output = model(images.to(device), masks.to(device), [target.to(device) for target in targets], oneStage)
                #print('edge', edge[0)
        ...
        for i in range(len(image_ids)):
            pic_id = '%s.png' % data_loader.dataset.ids[image_ids[i]]
...
def inference(model, data_loader, dataset_name, iou_types=("bbox",), box_only=False, device="cuda", expected_results=(), expected_results_sigma_tol=4, output_folder=None):
