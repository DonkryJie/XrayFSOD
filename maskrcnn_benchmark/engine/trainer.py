...
def do_train(model, data_loader, optimizer, scheduler, checkpointer, device, checkpoint_period, arguments, data_loader_val, oneStage, split, shot, test_period, cfg):
    ...
    for iteration, (images, targets, masks, _) in enumerate(data_loader, start_iter):
        ...
        loss_dict = model(images, masks, targets, oneStage)
        ...
        if data_loader_val is not None and test_period > 0 and iteration % test_period == 0:
            ...
            with torch.no_grad():
                for iteration_val, (images_val, targets_val, gtMasks_val, _) in enumerate(tqdm(data_loader_val)):
                    images_val = images_val.to(device)
                    gtMask_val = gtMasks_val.to(device)
                    targets_val = [target.to(device) for target in targets_val]


# images, masks, targets, oneStage
                    loss_dict = model(images_val, gtMask_val, targets_val, oneStage)
                    losses = sum(loss for loss in loss_dict.values())
                    loss_dict_reduced = reduce_loss_dict(loss_dict)
                    losses_reduced = sum(loss for loss in loss_dict_reduced.values())
